...
set_secret ASC_KEY_ID    "$(op read "op://$VAULT/asc-api-key/key-id")"
set_secret ASC_ISSUER_ID "$(op read "op://$VAULT/asc-api-key/issuer-id")"
# p8 base64-encoded so it survives as a single-line GitHub secret.
set_secret ASC_KEY_P8    "$(op read "op://$VAULT/asc-api-key/p8" | base64)"
