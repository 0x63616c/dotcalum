...
description: Use when Calum wants to install, reinstall, or update the dotcalum claude launch wrapper and pre-session lifecycle hook on a machine — symlinks the tracked wrapper/hook into ~/.claude and wires the shell so `claude` and `c` always run with --dangerously-skip-permissions.
---

# Install Claude Hooks

Installs the tracked claude launch wrapper + pre-session lifecycle hook from dotcalum.
...
Edit `claude/hooks/pre-session.sh` in the dotcalum repo (it's the symlink target, so edits are tracked in git). Put any command you want to run before every session there. It receives the launch args (`"$@"`) and runs best-effort — its exit code does not block the session.

## Files (all in dotcalum)
