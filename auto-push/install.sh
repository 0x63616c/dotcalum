#!/usr/bin/env bash
# Install the dotcalum auto-push LaunchAgent: every 5 min (and once at load/login),
...
PLIST="$LA_DIR/com.calum.dotcalum-autopush.plist"
mkdir -p "$LA_DIR" "$HOME/.local/state/autopush"

# Render the plist (use a temp file for __SSH_ENV__ since it spans multiple lines).
python3 - "$SCRIPT_DIR/launchd/com.calum.dotcalum-autopush.plist.template" "$PLIST" \
...
launchctl bootout "gui/$(id -u)/com.calum.dotcalum-autopush" 2>/dev/null || true
launchctl bootstrap "gui/$(id -u)" "$PLIST" 2>/dev/null \
  && echo "  loaded LaunchAgent com.calum.dotcalum-autopush (every 300s, RunAtLoad)" \
...
echo "Uninstall: launchctl bootout gui/$(id -u)/com.calum.dotcalum-autopush && rm $PLIST"
