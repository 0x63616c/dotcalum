...
#
# Pure bash; OURS is injected via DOTCALUM_HOOKS_DIR so the real global dir is never
...
# HEAL.
DOTCALUM_HOOKS_DIR="$gd" bash "$RECONCILE" --heal >/dev/null 2>&1
...
DOTCALUM_HOOKS_DIR="$gd" bash "$RECONCILE" --repo "$repo" quiet >/dev/null 2>&1
